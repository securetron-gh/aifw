...
### 1. Run the gateway and its database

```bash
docker compose up -d
```

The [`docker-compose.yml`](docker-compose.yml) in this repository starts the gateway alongside PostgreSQL: both share a network, the database runs on a named volume so data survives restarts, and the gateway waits for it to report healthy before starting. One command, no manual setup.

The default login is `superadmin` / `happy`.

> [!NOTE]
> The gateway listens on **80 (HTTP)** and **443 (HTTPS)**. Pin a published version with `AIFW_TAG=1.1.245 docker compose up -d`.

<details>
<summary>Running the container without Docker Compose</summary>

The image can be started on its own, but it does **not** provision a database, so you must supply a reachable PostgreSQL:

```bash
docker run -d --name aifw-gateway -p 8080:80 \
  -e "ConnectionStrings__PostgreSQL=Host=your-postgres;Port=5432;Database=aifw;Username=...;Password=..." \
  ghcr.io/securetron-gh/aifw:latest
```

> [!IMPORTANT]
> Without a reachable PostgreSQL the gateway still starts, but it falls back to in-memory stores and **all data is lost on restart**. The built-in default connection string points at `localhost`, which inside the container is the container itself, so a sibling database container must be addressed by its name on a shared Docker network.

The same image is published to Docker Hub as `securetron/aifw`.
...
Open the gateway at `http://localhost:8080` and watch the transaction appear in the dashboard.
